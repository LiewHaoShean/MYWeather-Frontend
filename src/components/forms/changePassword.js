...
import { changePasswordAction, generateOtpAction, loginUserAction } from "../../redux/slices/users/userSlice";
...
export default function ChangePassword(){
    ...
    const {loading, error, user, isOtpValidated, isPasswordChange} = useSelector((state)=> state?.users);

    useEffect(()=> {
        if(isPasswordChange){
            navigate("/login")
        }
    }, [dispatch, isPasswordChange]);
                ...
                <div className="relative sm:flex sm:flex-row justify-center h-96">
                    <div class="flex-col flex  self-center lg:px-14 sm:max-w-4xl xl:max-w-md absolute top-80 left-80 z-40">
                        <div class="self-start hidden lg:flex flex-col  text-gray-300">
                            <h1 class="my-3 font-semibold text-4xl">Reset your Password Now!</h1>
                            <p class="pr-3 text-sm opacity-75">Reminder: Jot down your password in a safe space.</p>
                        </div>
                    </div>
                    <div class="flex justify-center self-center absolute top-20 right-96 z-40 ">
                        <div class="p-12 bg-white mx-auto rounded-3xl w-96 shadow-2xl">
                            <div class="mb-7">
                                <h3 class="font-semibold text-2xl text-gray-800">Chnage Password </h3>
                                <p class="text-gray-400 font-semibold">Enter your new password below!</p>
                            </div>
                            <div class="space-y-6">
                                <div class="relative" x-data="{ show: true }">
                                    <input placeholder="Password" className="text-sm text-gray-200 px-4 py-3 rounded-lg w-full bg-gray-200 focus:bg-gray-100 border border-gray-200 focus:outline-none focus:border-sky-400" onChange={onChangeHandler} name="password" autoComplete="off" />
                                </div>
                                <div>
                                    {loading ? (<button type="submit" class="w-full flex justify-center bg-sky-800  hover:bg-sky-700 text-gray-100 p-3  rounded-lg tracking-wide font-semibold  cursor-pointer transition ease-in duration-500" onClick={submitBtnHandler}>
                                        <Stack sx={{ color: 'white' }} spacing={2} direction="row">
                                            <CircularProgress color="inherit" size={20}/>
                                        </Stack>
                                    </button>) : (
                                        <button type="submit" class="w-full flex justify-center bg-sky-800  hover:bg-sky-700 text-gray-100 p-3  rounded-lg tracking-wide font-semibold  cursor-pointer transition ease-in duration-500" onClick={submitBtnHandler}>Change</button>
                                    )}
                                </div>
