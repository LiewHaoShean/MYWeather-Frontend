...
export default function Login(){
    ...
    const submitBtnHandler = (e) => {
        e.preventDefault();
        dispatch(loginUserAction(dataForm));
    }

    const {loading, error, userInfo} = useSelector((state)=> state?.users?.userAuth);

    const {user, isPasswordChange} = useSelector((state)=> state?.users);
    ...
    return (
        <>
            {isPasswordChange && <SuccessNotification message={user?.message} time={50000}/>}
            <Navbar />
                    ...
                    <div class="flex justify-center self-center absolute top-20 right-96 z-40 ">
                        <div class="p-12 bg-white mx-auto rounded-3xl w-96 shadow-2xl">
                            <div class="mb-7">
                                ...
                                <p class="text-gray-400 font-semibold">Don't have an account? <a href="/register"
                                        class="text-sm text-sky-800 hover:text-sky-950 underline">Sign Up</a></p>
                            </div>
                            {/* login error */}
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="text-sm ml-auto">
                                        <Link to="/forgotPassword" class="text-sky-700 hover:text-sky-600">
                                ...
                                <div>
                                    ...
                                    <button type="submit" class="w-full flex justify-center bg-sky-800  hover:bg-sky-700 text-gray-100 p-3  rounded-lg tracking-wide font-semibold  cursor-pointer transition ease-in duration-500" onClick={submitBtnHandler}>
                                        ...
                                        <button type="submit" class="w-full flex justify-center bg-sky-800  hover:bg-sky-700 text-gray-100 p-3  rounded-lg tracking-wide font-semibold  cursor-pointer transition ease-in duration-500" onClick={submitBtnHandler}>Sign in</button>
                                    )}
                                </div>
